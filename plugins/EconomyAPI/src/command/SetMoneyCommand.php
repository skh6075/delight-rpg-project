...
final class SetMoneyCommand extends AwaitCommand implements PluginOwned{
	use PluginOwnedTrait;

	public function __construct(
		private readonly CurrencyRegistry $registry
	){
		parent::__construct("돈설정", "플레이어의 돈을 설정합니다.", "/돈설정 <플레이어> <금액> [화폐]");
		$this->setPermission("economyapi.command.set");
	...
	public function executeAsync(CommandSender $sender, string $commandLabel, array $args) : \Generator{
		...
		try{
			...
			$result = yield from Transaction::builder()
				...
				->reason("SetMoneyCommand: {$sender->getName()}")
				->build()
				->execute();
			if($result !== TransactionResult::SUCCESS){
				Message::alert($sender, "돈 설정에 실패했습니다: " . $result->value);
			}else{
				Message::success($sender, "§f{$name}§a님의 재화를 §f" . CurrencyFormatter::kindly($amount, $currency) . "§r§a 으(로) 설정했습니다.");
				StarGateExtension::broadcastPacket(MessagePacket::chat(
					content: Message::stringify("§f{$sender->getName()}§7님이 당신의 재화를 §f" . CurrencyFormatter::kindly($amount, $currency) . "§7 으(로) 설정했습니다."),
					targetId: $name
				));
			}
		}catch(\Throwable $e){
