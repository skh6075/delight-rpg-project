...
final class GiveMoneyCommand extends AwaitCommand implements PluginOwned{
	use PluginOwnedTrait;

	public function __construct(
		private readonly CurrencyRegistry $registry
	){
		parent::__construct("돈지급", "플레이어에게 돈을 지급합니다.", "/돈지급 <플레이어> <금액> [화폐]");
	...
	public function executeAsync(CommandSender $sender, string $commandLabel, array $args) : \Generator{
		if(count($args) < 2){
			Message::warn($sender, "사용법: §f/돈지급 <플레이어> <금액> [화폐]");
			return;
		}
		...
		try{
			...
			if($xuid === null){
				Message::alert($sender, "해당 플레이어를 찾을 수 없습니다.");
				return;
			}
			...
			$result = yield from Transaction::builder()
				...
				->reason("GiveMoneyCommand: {$sender->getName()}")
				->build()
				->execute();
			if($result !== TransactionResult::SUCCESS){
				Message::alert($sender, "돈 지급에 실패했습니다: " . $result->value);
			}else{
				Message::success($sender, "§f{$name}§a님에게 §f" . CurrencyFormatter::kindly($amount, $currency) . "§r§a 을(를) 지급했습니다.");
				StarGateExtension::broadcastPacket(MessagePacket::chat(
					content: Message::stringify("§f{$sender->getName()}§7님이 당신에게 §f" . CurrencyFormatter::kindly($amount, $currency) . "§7 을(를) 지급했습니다."),
					targetId: $name
				));
			}
		}catch(\Throwable $e){
